      ...
      children: [
        ...
        _FortuneHList(
          ...
          itemBuilder: (context, i) {
            ...
            final displayGz = isXiaoYun ? widget.table.fortune.getXiaoYunByAge(1) : d.ganZhi;

            return FortuneCard(
              shiShen: isXiaoYun ? '' : Relationship.getShiShen(widget.dayMaster, displayGz.gan).display,
              gz: displayGz,
              top: '${d.startTime.year}',
              bottom: isXiaoYun ? '1~${d.endAge}${'岁'.tr}' : '${d.startAge}${'虚岁'.tr}',
              isSel: selD == i,
        ...
        if (selD != null)
          _FortuneHList(
            ...
            itemBuilder: (context, i) {
              ...
              final GanZhi xiaoYunGz = widget.table.fortune.getXiaoYunByAge(age);
              final String bottomText = xiaoYunGz.toString();

              return FortuneCard(
                shiShen: Relationship.getShiShen(widget.dayMaster, y.ganZhi.gan).display,
                gz: y.ganZhi,
                top: '${y.year}',
                bottom: bottomText,
                isSel: selY == i,
        ...
        if (selD != null && selY != null)
          _FortuneHList(
            ...
            itemBuilder: (context, i) {
              ...
              return FortuneCard(
                shiShen: Relationship.getShiShen(widget.dayMaster, m.ganZhi.gan).display,
                gz: m.ganZhi,
                top: '${i + 1}${'月'.tr}',
                bottom: '流月'.tr,
                isSel: selM == i,
        ...
        if (selD != null && selY != null && selM != null)
          _FortuneHList(
            ...
            label: '流日'.tr,
            itemCount: widget.table.decades[selD].years[selY].months[selM].days.length,
            itemBuilder: (context, i) {
              final d = widget.table.decades[selD].years[selY].months[selM].days[i];
              final bool isToday = d.date.year == now.year && d.date.month == now.month && d.date.day == now.day;
              return FortuneCard(
                shiShen: Relationship.getShiShen(widget.dayMaster, d.ganZhi.gan).display,
                gz: d.ganZhi,
                top: '${d.date.day}${'日'.tr}',
                bottom: '流日'.tr,
                isSel: selDay == i,
                isCur: isToday,
                activeCol: Colors.blueAccent,
                onTap: () {
        ...
        if (selD != null && selY != null && selM != null && selDay != null)
          _FortuneHList(
            ...
            label: '流时'.tr,
            itemCount: widget.table.decades[selD].years[selY].months[selM].days[selDay].hours.length,
            itemBuilder: (context, i) {
              final dayObj = widget.table.decades[selD].years[selY].months[selM].days[selDay];
              final h = dayObj.hours[i];
              final bool isToday = dayObj.date.year == now.year && dayObj.date.month == now.month && dayObj.date.day == now.day;
              bool isCurrentHour = false;
              if (isToday) {
                final currentHourIdx = (now.hour + 1) ~/ 2 % 12;
                if (dayObj.ratHourMode != RatHourMode.noSplit) {
                  if (now.hour >= 23) isCurrentHour = (i == 12);
                  else isCurrentHour = (h.hourIndex == currentHourIdx && i < 12);
                } else {
                  isCurrentHour = (h.hourIndex == currentHourIdx);
                }
              }
              return FortuneCard(
                shiShen: Relationship.getShiShen(widget.dayMaster, h.ganZhi.gan).display,
                gz: h.ganZhi,
                top: h.name.tr,
                bottom: '时辰'.tr,
                isSel: ref.watch(selHourIdxProvider) == i,
                isCur: isCurrentHour,
