...
建立在极致解耦、纯 Dart 编写的三大核心引擎之上（`sxwnl_spa_dart`, `bazi_core`, `ziwei_core`），OpenDestiny 能够提供长达 6,000 年（公元前 1000 年至公元 5000 年）的瑞士星历表级真太阳时校准，为你呈现一个无状态的、极速的、且完全本地化离线运算的专业排盘体验。

这不仅是一个开箱即用的应用，更是整个浩瀚而严谨的底层星命引擎在业务层的最佳 UI 实践标杆。
...
1.  📦 **[`sxwnl_spa_dart`](https://pub.dev/packages/sxwnl_spa_dart)** (v0.16.0) - 基于瑞士精确星历表及太阳视运动规律的天文学最底层工具库。
